...
class Network_freq(object):
    ...
    def _add_gt_image_summary(self):
        ...
        image = tf.py_func(draw_bounding_boxes,
                           [self._gt_image, self._gt_boxes, self._gt_labels],
    ...
    def _region_proposal(self, net_conv, is_training, initializer):
        ...
        rpn_cls_score = slim.conv2d(rpn, self._num_anchors * 2, [1, 1], trainable=is_training,
                                    ...
                                    padding='VALID', activation_fn=None, scope='rpn_cls_score')  # N*h*w*18
        # change it so that the score has 2 as its channel size
        rpn_cls_score_reshape = self._reshape_layer(rpn_cls_score, 2, 'rpn_cls_score_reshape')  # 1*9Nh*w*2
        rpn_cls_prob_reshape = self._softmax_layer(rpn_cls_score_reshape, "rpn_cls_prob_reshape")
        rpn_cls_pred = tf.argmax(tf.reshape(rpn_cls_score_reshape, [-1, 2]), axis=1, name="rpn_cls_pred")
        rpn_cls_prob = self._reshape_layer(rpn_cls_prob_reshape, self._num_anchors * 2, "rpn_cls_prob")  # 1*Nh*w*18
        rpn_bbox_pred = slim.conv2d(rpn, self._num_anchors * 4, [1, 1], trainable=is_training,
                                    weights_initializer=initializer,
                                    padding='VALID', activation_fn=None, scope='rpn_bbox_pred')  # N*h*w*36
        if is_training:
    ...
    def _parse_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                     self._gt_boxes: np.hstack((blobs['gt_boxes'], blobs['gt_labels'])),
                     self._gt_labels: blobs['gt_labels']}
        return feed_dict

    def _parse_test_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'], self._rois: blobs['rois']}
        return feed_dict

    # only useful during testing mode
    def test_image(self, sess, blobs):
        cls_score, cls_prob, bbox_pred = sess.run([self._predictions["cls_score"],
                                                         self._predictions['cls_prob'],
                                                         self._predictions['bbox_pred']],
                                                        feed_dict=self._parse_test_dict(blobs))
        return cls_score, cls_prob, bbox_pred

    def get_rpn(self, sess, blobs):
        rpn_boxes, rpn_scores = sess.run([self._predictions['rois'], self._predictions['roi_scores']],
                                         feed_dict=self._parse_test_dict(blobs))
    ...
    def train_step(self, sess, blobs, train_op):
        out = sess.run([self._losses["rpn_cross_entropy"],
        ...
        return {'loss_rpn_cls': out[0], 'loss_rpn_box': out[1], 'loss_cls': out[2], 'loss_box': out[3],
                'loss_total': out[4]}
    ...
    def train_step_with_summary(self, sess, blobs, train_op):
        out = sess.run([self._losses["rpn_cross_entropy"],
                       ...
                       feed_dict=self._parse_dict(blobs))
        return {'loss_rpn_cls': out[0], 'loss_rpn_box': out[1], 'loss_cls': out[2], 'loss_box': out[3],
                'loss_total': out[4], 'summary': out[5]}
