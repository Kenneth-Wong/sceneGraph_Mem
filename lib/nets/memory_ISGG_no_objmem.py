...
class Memory_ISGG_no_objmem(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
    ...
    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
        ...
        if iter == 0:
            ...
            mem_fc7_ld, rel_mem_fc7_ld = self._fc_init(None, rel_mem_fc7, is_training, "unary") # reuse
            vert_factor_new = vert_factor
        ...
        for iter in range(cfg.MEM.ITER):
            ...
            with tf.variable_scope(self._scope, reuse=reuse):
                ...
                rel_cls_score, rel_cls_prob, rel_cls_pred, vert_factor, edge_factor = \
                    self._build_pred_all(is_training, None, cls_score_conv, bbox_pred_conv, rois, batch_ids,
                                         rel_mem, rel_score_conv, rel_rois, rel_batch_ids, vert_factor, edge_factor, iter)
...
class vgg16(Memory_ISGG_no_objmem, vgg16):
    def __init__(self):
        Memory_ISGG_no_objmem.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(Memory_ISGG_no_objmem, resnetv1):
...
class mobilenetv1(Memory_ISGG_no_objmem, mobilenetv1):
    def __init__(self):
        Memory_ISGG_no_objmem.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
