...
def get_inv_item(inv_rois, inv_batch_ids, i):
    return inv_rois[i, :][None], inv_batch_ids[i][None]
...
class BaseMemory(Network):
    ...
    def _build_pred_rel_deprecated(self, is_training, obj_mem_rel_pool5_nb, rel_mem, pred_rel_rois, pred_rel_batch_ids,
                                   iter):
    ...
    def _add_memory_losses(self, name):
        ...
        assert len(self._predictions["obj_pool_map"]) == cfg.MEM.ITER - 1
        assert len(self._predictions["fg_obj_global_map"]) == cfg.MEM.ITER - 1
        assert len(self._predictions["rel_pool_map"]) == cfg.MEM.ITER - 1
        with tf.variable_scope(name):
            ...
            # rel_map_targets = self._targets["target_rel_map"]
            # obj_map_targets = self._targets["target_obj_map"]
            ...
            for iter in range(cfg.MEM.ITER):
                ...
                '''
                if iter < cfg.MEM.ITER - 1:
                    obj_pool_map = self._predictions["obj_pool_map"][iter]
                    obj_map_loss.append(self._l2_loss(obj_pool_map, obj_map_targets))

                    rel_pool_map = self._predictions["rel_pool_map"][iter]
                    rel_map_loss.append(self._l2_loss(rel_pool_map, rel_map_targets))
                '''
                '''
                # tag loss
            ...
            # self._losses['obj_map_loss'] = tf.zeros([1]) #tf.reduce_mean(obj_map_loss, name="obj_map_loss")
            # self._losses['rel_map_loss'] = tf.zeros([1]) #tf.reduce_mean(rel_map_loss, name="rel_map_loss")
    ...
    def _create_summary(self):
        ...
        with tf.device("/cpu:0"):
            ...
            for iter in range(cfg.MEM.ITER):
                ...
                if iter > 0:
                    val_summaries.append(self._add_CAM_summary(iter - 1))
                    val_summaries.append(self._add_rel_CAM_summary(iter - 1))
...
class resnetv1(BaseMemory, resnetv1):
...
class mobilenetv1(BaseMemory, mobilenetv1):
    def __init__(self):
        BaseMemory.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
