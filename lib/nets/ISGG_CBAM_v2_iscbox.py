...
class Memory_ISGG_CBAM_v2_iscbox(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
    ...
    def _channel_communicate(self, vert_channel_factor, edge_channel_factor, vert_bottom, edge_bottom, is_training,
        ...
        vert_channel_info = tf.expand_dims(tf.expand_dims(vert_channel_factor, 1), 1,
                                              name="vert_channel_info")
        edge_channel_info = tf.expand_dims(tf.expand_dims(edge_channel_factor, 1), 1,
                                              name="edge_channel_info")
        vert_channel_atten = tf.multiply(vert_bottom, vert_channel_info, name="vert_channel_atten")
        edge_channel_atten = tf.multiply(edge_bottom, edge_channel_info, name="edge_channel_atten")
    ...
    def _spatial_communicate(self, vert_spatial_factor, edge_spatial_factor, vert_bottom, edge_bottom, is_training,
        ...
        vert_spatial_atten = tf.multiply(vert_bottom, vert_spatial_factor, name="vert_spatial_atten")
        edge_spatial_atten = tf.multiply(edge_bottom, edge_spatial_factor, name="edge_spatial_atten")
    ...
    def _build_conv(self, is_training):
        ...
        rel_fc7 = self._head_to_tail(edge_atten2, is_training, prefix='rel')

        # First iteration
        with tf.variable_scope(self._scope, self._scope):
            # region classification
            cls_score_conv, cls_prob_conv = self._cls_init(vert_fc7, is_training)
            bbox_pred_conv = self._reg_init(vert_fc7, is_training)

        with tf.variable_scope(self._scope, self._scope):
            rel_score_conv, rel_prob_conv = self._cls_init(rel_fc7, is_training, prefix='rel')
    ...
    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
        ...
        if iter == 0:
            ...
            rel_fc7 = self._head_to_tail(edge_atten2, is_training, prefix='rel', open_scope=False)
            #edge_gap = tf.reduce_mean(edge_atten2, axis=[1, 2])
            cls_score_mem = self._cls_iter(vert_fc7, is_training, "cls_iter", iter)
            bbox_pred_mem = self._bbox_iter(vert_fc7, is_training, "bbox_iter", iter)
            rel_score_mem = self._rel_cls_iter(rel_fc7, is_training, "rel_cls_iter", iter)
    ...
    def _parse_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                     self._rois: blobs['rois'], self._rel_rois: blobs['isc_rois'], self._isc_rois: blobs['isc_rois'],
...
class vgg16(Memory_ISGG_CBAM_v2_iscbox, vgg16):
    def __init__(self):
        Memory_ISGG_CBAM_v2_iscbox.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(Memory_ISGG_CBAM_v2_iscbox, resnetv1):
...
class mobilenetv1(Memory_ISGG_CBAM_v2_iscbox, mobilenetv1):
    def __init__(self):
        Memory_ISGG_CBAM_v2_iscbox.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
