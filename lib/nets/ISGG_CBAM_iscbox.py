...
from utils.isgg_utils import pad_and_gather
from utils.visualization import draw_predicted_boxes, draw_memory, draw_CAM


class Memory_ISGG_CBAM_iscbox(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
        ...
        self._channel_vert_state_dim = 512
        self._channel_edge_state_dim = 512
        self._spatial_vert_state_dim = cfg.MEM.CROP_SIZE * cfg.MEM.CROP_SIZE
        self._spatial_edge_state_dim = cfg.MEM.CROP_SIZE * cfg.MEM.CROP_SIZE
    ...
    def _cls_init(self, bottom, is_training, prefix=None):
        out = self._num_classes if prefix is None else self._num_predicates
        prefix = '' if prefix is None else prefix + '_'
        initializer = tf.random_normal_initializer(mean=0.0, stddev=0.01)
        bottom_flat = slim.flatten(bottom, scope=prefix + 'cls_flatten')
        cls_score = slim.fully_connected(bottom_flat, out,
    ...
    def _reg_init(self, bottom, is_training):
        initializer = tf.random_normal_initializer(mean=0.0, stddev=0.01)
        bottom_flat = slim.flatten(bottom, scope='reg_flatten')
        bbox_pred = slim.fully_connected(bottom_flat, self._num_classes * 4,
                                         weights_initializer=initializer,
                                         trainable=is_training,
    ...
    def _cls_iter(self, bottom, is_training, name, iter):
        initializer = tf.random_normal_initializer(mean=0.0, stddev=cfg.MEM.C_STD)
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            bottom_flat = slim.flatten(bottom, scope='cls_flatten')
            cls_score_mem = slim.fully_connected(bottom_flat, self._num_classes,
                                                 weights_initializer=initializer,
                                                 activation_fn=None,
                                                 trainable=is_training,
                                                 biases_initializer=tf.constant_initializer(0.0),
    ...
    def _cells(self):
        ...
        """
        # intiialize lstms
        self.channel_vert_rnn = tf.nn.rnn_cell.GRUCell(self._channel_vert_state_dim, activation=tf.tanh)
        self.channel_edge_rnn = tf.nn.rnn_cell.GRUCell(self._channel_edge_state_dim, activation=tf.tanh)
        self.spatial_vert_rnn = tf.nn.rnn_cell.GRUCell(self._spatial_vert_state_dim, activation=tf.tanh)
        self.spatial_edge_rnn = tf.nn.rnn_cell.GRUCell(self._spatial_edge_state_dim, activation=tf.tanh)

        # lstm states
        self.channel_vert_state = self.channel_vert_rnn.zero_state(self._num_roi, tf.float32)
        self.channel_edge_state = self.channel_edge_rnn.zero_state(self._num_rel, tf.float32)
        self.spatial_vert_state = self.spatial_vert_rnn.zero_state(self._num_roi, tf.float32)
        self.spatial_edge_state = self.spatial_edge_rnn.zero_state(self._num_rel, tf.float32)

    def _channel_attention(self, bottom, is_training, name):
        xavier = tf.contrib.layers.variance_scaling_initializer()
        with tf.variable_scope(name):
            avg_pool5 = tf.reduce_mean(bottom, axis=[1, 2], name="avg_pool5")  # N * 512
            max_pool5 = tf.reduce_max(bottom, axis=[1, 2], name="max_pool5")  # N * 512
            with slim.arg_scope([slim.fully_connected],
                                activation_fn=None,
                                trainable=is_training,
                                weights_initializer=xavier,
                                reuse=tf.AUTO_REUSE,
                                biases_initializer=tf.constant_initializer(0.0)):
                avg_pool5_mlp = slim.fully_connected(
                    slim.fully_connected(avg_pool5, cfg.REDUCE_RATIO, scope="channel_mlp1"),
                    cfg.MEM.C, scope="channel_mlp2")
                max_pool5_mlp = slim.fully_connected(
                    slim.fully_connected(max_pool5, cfg.REDUCE_RATIO, scope="channel_mlp1"),
                    cfg.MEM.C, scope="channel_mlp2")
            channel_mlp = avg_pool5_mlp + max_pool5_mlp
        return channel_mlp

    def _channel_communicate(self, vert_channel_factor, edge_channel_factor, vert_bottom, edge_bottom, is_training,
                             reuse):
        edge_ctx = self._compute_edge_context(vert_channel_factor, edge_channel_factor, is_training, atten="channel",
                                              reuse=reuse)
        edge_channel_factor = self._rnn_forward(edge_ctx, rnn="edge", atten="channel", name="edge_channel_rnn",
                                                reuse=True)
        vert_ctx = self._compute_vert_context(edge_channel_factor, vert_channel_factor, is_training, atten="channel",
                                              reuse=reuse)
        vert_channel_factor = self._rnn_forward(vert_ctx, rnn="vert", atten="channel", name="vert_channel_rnn",
                                                reuse=True)

        vert_channel_weights = tf.expand_dims(tf.expand_dims(tf.nn.sigmoid(vert_channel_factor), 1), 1,
                                              name="vert_channel_weights")
        edge_channel_weights = tf.expand_dims(tf.expand_dims(tf.nn.sigmoid(edge_channel_factor), 1), 1,
                                              name="edge_channel_weights")
        vert_channel_atten = tf.multiply(vert_bottom, vert_channel_weights, name="vert_channel_atten")
        edge_channel_atten = tf.multiply(edge_bottom, edge_channel_weights, name="edge_channel_atten")
        return vert_channel_atten, edge_channel_atten, vert_channel_factor, edge_channel_factor

    def _spatial_communicate(self, vert_spatial_factor, edge_spatial_factor, vert_bottom, edge_bottom, is_training,
                             reuse):
        ori_shape = [-1, cfg.MEM.CROP_SIZE, cfg.MEM.CROP_SIZE, 1]
        linear_shape = [-1, cfg.MEM.CROP_SIZE * cfg.MEM.CROP_SIZE]
        edge_ctx = self._compute_edge_context(tf.reshape(vert_spatial_factor, shape=linear_shape),
                                              tf.reshape(edge_spatial_factor, shape=linear_shape),
                                              is_training, atten="spatial", reuse=reuse)
        edge_spatial_factor = self._rnn_forward(edge_ctx, rnn="edge", atten="spatial", name="edge_spatial_rnn",
                                                reuse=True)
        vert_ctx = self._compute_vert_context(tf.reshape(edge_spatial_factor, shape=linear_shape),
                                              tf.reshape(vert_spatial_factor, shape=linear_shape),
                                              is_training, atten="spatial", reuse=reuse)
        vert_spatial_factor = self._rnn_forward(vert_ctx, rnn="vert", atten="spatial", name="vert_spatial_rnn",
                                                reuse=True)

        vert_spatial_weights = tf.nn.sigmoid(vert_spatial_factor, name="vert_spatial_weights")
        edge_spatial_weights = tf.nn.sigmoid(edge_spatial_factor, name="edge_spatial_weights")
        vert_spatial_atten = tf.multiply(vert_bottom, vert_spatial_weights, name="vert_spatial_atten")
        edge_spatial_atten = tf.multiply(edge_bottom, edge_spatial_weights, name="edge_spatial_atten")
        return vert_spatial_atten, edge_spatial_atten, vert_spatial_factor, edge_spatial_factor

    def _spatial_attention(self, bottom, is_training, name):
        xavier = tf.contrib.layers.variance_scaling_initializer()
        with tf.variable_scope(name):
            avg_pool5 = tf.reduce_mean(bottom, axis=3, keep_dims=True, name="avg_pool5")
            max_pool5 = tf.reduce_max(bottom, axis=3, keep_dims=True, name="max_pool5")
            aggregated_pool5 = tf.concat([avg_pool5, max_pool5], axis=3)
            with slim.arg_scope([slim.conv2d],
                                activation_fn=None,
                                trainable=is_training,
                                weights_initializer=xavier,
                                biases_initializer=tf.constant_initializer(0.0)):
                spatial_attention = slim.conv2d(aggregated_pool5, 1, [cfg.ATTENTION_KSIZE, cfg.ATTENTION_KSIZE])
        return spatial_attention

    def _rnn_forward(self, attention, rnn="vert", atten="channel", name="vert_channel_rnn", reuse=False):
        names = self.__dict__
        with tf.variable_scope(name):
            if reuse: tf.get_variable_scope().reuse_variables()
            rnn_function = names[atten + '_' + rnn + '_rnn']
            state = names[atten + '_' + rnn + '_state']
            if atten == 'spatial':
                ori_shape = [-1, cfg.MEM.CROP_SIZE, cfg.MEM.CROP_SIZE, 1]
                linear_shape = [-1, cfg.MEM.CROP_SIZE * cfg.MEM.CROP_SIZE]
                out, names[atten + '_' + rnn + '_state'] = rnn_function(tf.reshape(attention, shape=linear_shape),
                                                                        state)
                out = tf.reshape(out, shape=ori_shape)
            else:
                out, names[atten + '_' + rnn + '_state'] = rnn_function(attention, state)
        return out
    ...
    def _build_conv(self, is_training):
        ...
        with tf.variable_scope(self._scope, self._scope):
            vert_channel = self._channel_attention(pool5, is_training, "vert_channel")  # N * 512
            edge_channel = self._channel_attention(rel_pool5, is_training, "edge_channel")  # N * 512
            vert_channel_factor = self._rnn_forward(vert_channel, rnn="vert", atten="channel",
                                                    name="vert_channel_rnn", reuse=False)
            edge_channel_factor = self._rnn_forward(edge_channel, rnn="edge", atten="channel",
                                                    name="edge_channel_rnn", reuse=False)
            vert_atten1, edge_atten1, vert_channel_factor, edge_channel_factor = \
                self._channel_communicate(vert_channel_factor, edge_channel_factor, pool5, rel_pool5, is_training,
                                          reuse=False)
            vert_spatial = self._spatial_attention(vert_atten1, is_training, "vert_spatial")  # N * 7 * 7 * 1
            edge_spatial = self._spatial_attention(edge_atten1, is_training, "edge_spatial")  # N * 7 * 7 * 1
            vert_spatial_factor = self._rnn_forward(vert_spatial, rnn="vert", atten="spatial",
                                                    name="vert_spatial_rnn", reuse=False)
            edge_spatial_factor = self._rnn_forward(edge_spatial, rnn="edge", atten="spatial",
                                                    name="edge_spatial_rnn", reuse=False)
            vert_atten2, edge_atten2, vert_spatial_factor, edge_spatial_factor = \
                self._spatial_communicate(vert_spatial_factor, edge_spatial_factor, vert_atten1, edge_atten1,
                                          is_training, reuse=False)

            vert_gap = tf.reduce_mean(vert_atten2, axis=[1, 2])
            edge_gap = tf.reduce_mean(edge_atten2, axis=[1, 2])

        #vert_fc7 = self._head_to_tail(vert_atten2, is_training)

        # First iteration
        with tf.variable_scope(self._scope, self._scope):
            # region classification
            cls_score_conv, cls_prob_conv = self._cls_init(vert_gap, is_training)
            bbox_pred_conv = self._reg_init(vert_gap, is_training)

        with tf.variable_scope(self._scope, self._scope):
            rel_score_conv, rel_prob_conv = self._cls_init(edge_gap, is_training, prefix='rel')
        ...
        return cls_score_conv, bbox_pred_conv, pool5_nb, rel_score_conv, rel_pool5_nb, \
               ...
               isc_rois, isc_batch_ids, inv_isc_rois, inv_isc_batch_ids, vert_channel_factor, edge_channel_factor, \
               vert_spatial_factor, edge_spatial_factor

    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
                        rel_mem, rel_score_conv, rel_rois, rel_batch_ids, isc_rois, isc_batch_ids,
                        vert_channel_factor, edge_channel_factor, vert_spatial_factor, edge_spatial_factor, iter):
        if cfg.MEM.CT_L:
            ...
            mem_net = mem
            rel_mem_net = rel_mem
        mem_ct_from_obj_pool5 = self._crop_rois(mem_net, rois, batch_ids, "mem_ct_from_obj_pool5",
                                                iter)  # N * 7 * 7 * 512
        mem_ct_from_rel_pool5 = self._crop_rois(rel_mem_net, rois, batch_ids, "mem_ct_from_rel_pool5", iter)
        mem_ct_pool5 = tf.add(mem_ct_from_obj_pool5, mem_ct_from_rel_pool5, "mem_ct_pool5")
        rel_mem_ct_from_rel_pool5 = self._crop_rois(rel_mem_net, rel_rois, rel_batch_ids, "rel_mem_ct_from_rel_pool5",
                                                    iter)
        rel_mem_ct_from_obj_pool5 = self._crop_rois(mem_net, isc_rois, isc_batch_ids, "rel_mem_ct_from_obj_pool5", iter)
        rel_mem_ct_pool5 = tf.add(rel_mem_ct_from_rel_pool5, rel_mem_ct_from_obj_pool5, "rel_mem_ct_pool5")
        if iter == 0:
            cls_score_mem = None
            bbox_pred_mem = None
            rel_score_mem = None
            ...
            vert_channel_mem = self._channel_attention(mem_ct_pool5, is_training, "vert_channel")  # N * 512
            edge_channel_mem = self._channel_attention(rel_mem_ct_pool5, is_training, "edge_channel")  # N * 512
            vert_channel_factor_comb = tf.add(u * vert_channel_factor, v * vert_channel_mem,
                                              name="vert_channel_factor_comb")
            edge_channel_factor_comb = tf.add(p * edge_channel_factor, q * edge_channel_mem,
                                              name="edge_channel_factor_comb")
            vert_atten1, edge_atten1, vert_channel_factor, edge_channel_factor = self._channel_communicate(
                vert_channel_factor_comb, edge_channel_factor_comb,
                mem_ct_pool5, rel_mem_ct_pool5, is_training,
                reuse=True)
            vert_spatial_mem = self._spatial_attention(vert_atten1, is_training, "vert_spatial")
            edge_spatial_mem = self._spatial_attention(edge_atten1, is_training, "edge_spatial")
            vert_spatial_factor_comb = tf.add(u * vert_spatial_factor, v * vert_spatial_mem,
                                              name="vert_spatial_factor_comb")
            edge_spatial_factor_comb = tf.add(p * edge_spatial_factor, q * edge_spatial_mem,
                                              name="edge_spatial_factor_comb")
            vert_atten2, edge_atten2, vert_spatial_factor, edge_spatial_factor = self._spatial_communicate(
                vert_spatial_factor_comb, edge_spatial_factor_comb,
                vert_atten1, edge_atten1, is_training, reuse=True)
            vert_gap = tf.reduce_mean(vert_atten2, axis=[1, 2])
            #vert_fc7 = self._head_to_tail(vert_atten2, is_training, open_scope=False)
            edge_gap = tf.reduce_mean(edge_atten2, axis=[1, 2])
            cls_score_mem = self._cls_iter(vert_gap, is_training, "cls_iter", iter)
            bbox_pred_mem = self._bbox_iter(vert_gap, is_training, "bbox_iter", iter)
            rel_score_mem = self._rel_cls_iter(edge_gap, is_training, "rel_cls_iter", iter)
        ...
        return cls_score, cls_prob, cls_pred, bbox_pred, rel_cls_score, rel_cls_prob, rel_cls_pred, \
               vert_channel_factor, edge_channel_factor, vert_spatial_factor, edge_spatial_factor
    ...
    def _build_memory(self, is_training, is_testing):
        ...
        rois, batch_ids, inv_rois, inv_batch_ids, rel_rois, rel_batch_ids, inv_rel_rois, inv_rel_batch_ids, \
        isc_rois, isc_batch_ids, inv_isc_rois, inv_isc_batch_ids, vert_channel_factor, edge_channel_factor, \
        vert_spatial_factor, edge_spatial_factor = self._build_conv(is_training)
        ...
        for iter in range(cfg.MEM.ITER):
            ...
            with tf.variable_scope(self._scope, reuse=reuse):
                ...
                rel_cls_score, rel_cls_prob, rel_cls_pred, vert_channel_factor, edge_channel_factor, \
                vert_spatial_factor, edge_spatial_factor = \
    ...
    def create_architecture(self, mode, num_classes, num_predicates, tag=None):
        ...
        self._roi_pair_mask_inds = tf.placeholder(tf.int32, shape=[None, 2])
    ...
    def _parse_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                     self._rois: blobs['rois'], self._rel_rois: blobs['rel_rois'], self._isc_rois: blobs['isc_rois'],
                     ...
                     self._roi_pair_mask_inds: blobs['roi_pair_mask_inds']
                     }
        return feed_dict
...
class vgg16(Memory_ISGG_CBAM_iscbox, vgg16):
    def __init__(self):
        Memory_ISGG_CBAM_iscbox.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(Memory_ISGG_CBAM_iscbox, resnetv1):
...
class mobilenetv1(Memory_ISGG_CBAM_iscbox, mobilenetv1):
    def __init__(self):
        Memory_ISGG_CBAM_iscbox.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
