...
class Attend_Memory_ISGG_isc(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
    ...
    def _confidence_iter(self, mem_fc7, is_training, name, iter, prefix=None):
        initializer = tf.random_normal_initializer(mean=0.0, stddev=cfg.MEM.C_STD)
        prefix = '' if prefix is None else prefix + '_'
        with tf.variable_scope(name):
            confid_mem = slim.fully_connected(mem_fc7, 1,
                                              ...
                                              scope=prefix + "confid_mem")
            self._predictions[prefix + 'confid'].append(confid_mem)
    ...
    def _aggregate_pred(self, name):
        with tf.variable_scope(name):
            ...
            rel_comb_confid = tf.stack(self._predictions['rel_confid'], axis=2, name='rel_comb_confid')
            rel_comb_attend = tf.nn.softmax(rel_comb_confid, dim=2, name='rel_comb_attend')
            self._predictions['rel_confid_prob'] = tf.unstack(rel_comb_attend, axis=2, name='rel_unstack_attend')
            ...
            comb_bbox = tf.stop_gradient(tf.stack(self._predictions["bbox_pred"], axis=2, name='comb_bbox'),
                                         name="comb_bbox_nb")
            bbox_pred = tf.reduce_sum(tf.multiply(comb_bbox, comb_attend, name="weighted_bbox_pred"), axis=2,
                                      name="attend_bbox_pred")

            rel_comb_score = tf.stop_gradient(tf.stack(self._predictions["rel_cls_score"],
                                                       axis=2, name='rel_comb_score'),
                                              name="rel_comb_score_nb")
            rel_cls_score = tf.reduce_sum(tf.multiply(rel_comb_score, rel_comb_attend, name='weighted_rel_cls_score'),
                                          axis=2, name='attend_rel_cls_score')
            rel_cls_prob = tf.nn.softmax(rel_cls_score, name="attend_rel_cls_prob")
            rel_cls_pred = tf.argmax(cls_score, axis=1, name="attend_rel_cls_pred")
            ...
            self._predictions["attend_bbox_pred"] = bbox_pred
            self._predictions['attend'] = self._predictions['confid_prob']

            self._predictions["attend_rel_cls_score"] = rel_cls_score
            self._predictions["attend_rel_cls_prob"] = rel_cls_prob
            self._predictions["attend_rel_cls_pred"] = rel_cls_pred
            self._predictions['rel_attend'] = self._predictions['rel_confid_prob']

        return cls_prob, rel_cls_prob, bbox_pred
    ...
    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
        ...
        self._confidence_iter(rel_mem_fc7, is_training, "rel_confid_iter", iter, prefix="rel")
    ...
    def _build_memory(self, is_training, is_testing):
        ...
        for iter in range(cfg.MEM.ITER):
            ...
            with tf.variable_scope(self._scope, reuse=reuse):
                ...
                rel_cls_score, rel_cls_prob, rel_cls_pred, vert_factor, edge_factor = \
                    self._build_pred_all(is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
                                         rel_mem, rel_score_conv, rel_rois, rel_batch_ids, vert_factor, edge_factor,
                                         iter)
            ...
            if iter == 0:
                reuse = True
        cls_prob, rel_cls_prob, bbox_pred = self._aggregate_pred("aggregate")
    ...
    def _add_memory_losses(self, name):
        ...
        with tf.variable_scope(name):
            ...
            self._losses["cross_entropy_final"] = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(logits=cls_score,
                                                               labels=label))
            bbox_pred = self._predictions["attend_bbox_pred"]
            self._losses["bbox_loss_final"] = self._smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights,
                                                                   bbox_outside_weights)

            rel_cls_score = self._predictions["attend_rel_cls_score"]
            self._losses["rel_cross_entropy_final"] = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(logits=rel_cls_score,
                                                               labels=predicate))
            ...
            self._losses['bbox_loss'] = self._losses['bbox_loss_image'] \
                                        ...
                                        cfg.MEM.WEIGHT_FINAL * self._losses['bbox_loss_final']
            ...
            self._losses['rel_cross_entropy'] = self._losses['rel_cross_entropy_image'] \
                                                ...
                                                cfg.MEM.WEIGHT_FINAL * self._losses['rel_cross_entropy_final']
    ...
    def _parse_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                     self._rois: blobs['rois'], self._rel_rois: blobs['isc_rois'],
    ...
    def test_image(self, sess, blobs):
        cls_score, cls_prob, bbox_pred, rel_cls_score, rel_cls_prob = sess.run([self._predictions["attend_cls_score"],
                                                                                self._predictions['attend_cls_prob'],
                                                                                self._predictions['attend_bbox_pred'],
                                                                                self._predictions['attend_rel_cls_score'],
                                                                                self._predictions['attend_rel_cls_prob']],
...
class vgg16(Attend_Memory_ISGG_isc, vgg16):
    def __init__(self):
        Attend_Memory_ISGG_isc.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(Attend_Memory_ISGG_isc, resnetv1):
...
class mobilenetv1(Attend_Memory_ISGG_isc, mobilenetv1):
    def __init__(self):
        Attend_Memory_ISGG_isc.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
