...
class alt_Memory_ISGG(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
    ...
    def _cells(self):
        ...
        self.vert_factor_rnn = tf.nn.rnn_cell.GRUCell(self._vert_state_dim, activation=tf.tanh)
        self.edge_factor_rnn = tf.nn.rnn_cell.GRUCell(self._edge_state_dim, activation=tf.tanh)
        self.vert_factor_state = None
        self.edge_factor_state = None

    def _vert_factor_rnn_forward(self, vert_factor_in, name, reuse=tf.AUTO_REUSE):
        assert self.vert_factor_state is not None
        with tf.variable_scope(name, reuse=reuse):
            (vert_factor_out, self.vert_factor_state) = self.vert_factor_rnn(vert_factor_in, self.vert_factor_state)
        return vert_factor_out

    def _edge_factor_rnn_forward(self, edge_factor_in, name, reuse=tf.AUTO_REUSE):
        assert self.edge_factor_state is not None
        with tf.variable_scope(name, reuse=reuse):
            (edge_factor_out, self.edge_factor_state) = self.edge_factor_rnn(edge_factor_in, self.edge_factor_state)
        return edge_factor_out
    ...
    def _build_conv(self, is_training):
        ...
        # initialize the factor state
        self.vert_factor_state = vert_factor
        self.edge_factor_state = edge_factor
    ...
    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
        ...
        if iter == 0:
            ...
            vert_unary, edge_unary = self._fc_init(mem_fc7, rel_mem_fc7, is_training, "unary") # reuse
            vert_factor = self._vert_factor_rnn_forward(vert_unary, "vert_factor_rnn")
            edge_factor = self._edge_factor_rnn_forward(edge_unary, "edge_factor_rnn")
            edge_ctx = self._compute_edge_context(vert_factor, edge_factor, is_training, reuse=True)
            edge_factor = self._edge_rnn_forward(edge_ctx, "edge_rnn", reuse=True)
            vert_ctx = self._compute_vert_context(edge_factor, vert_factor, is_training, reuse=True)
            vert_factor = self._vert_rnn_forward(vert_ctx, "vert_rnn", reuse=True)
...
class vgg16(alt_Memory_ISGG, vgg16):
    def __init__(self):
        alt_Memory_ISGG.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(alt_Memory_ISGG, resnetv1):
...
class mobilenetv1(alt_Memory_ISGG, mobilenetv1):
    def __init__(self):
        alt_Memory_ISGG.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
