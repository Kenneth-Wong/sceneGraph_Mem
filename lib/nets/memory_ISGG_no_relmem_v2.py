...
class Memory_ISGG_no_relmem_v2(DualGraph, Network):
    def __init__(self):
        self._predictions = {}
    ...
    def _build_pred_all(self, is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
        ...
        if iter == 0:
            ...
            mem_fc7_ld, _ = self._fc_init(mem_fc7, None, is_training, "unary") # reuse
            vert_factor_new = tf.add(u * vert_factor, v * mem_fc7_ld, name="vert_factor_new")
            edge_factor_new = edge_factor
            ...
            with tf.variable_scope(self._scope, reuse=reuse):
                ...
                rel_cls_score, rel_cls_prob, rel_cls_pred, vert_factor, edge_factor = \
                    self._build_pred_all(is_training, mem, cls_score_conv, bbox_pred_conv, rois, batch_ids,
                                         None, rel_score_conv, rel_rois, rel_batch_ids, vert_factor, edge_factor, iter)
...
class vgg16(Memory_ISGG_no_relmem_v2, vgg16):
    def __init__(self):
        Memory_ISGG_no_relmem_v2.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(Memory_ISGG_no_relmem_v2, resnetv1):
...
class mobilenetv1(Memory_ISGG_no_relmem_v2, mobilenetv1):
    def __init__(self):
        Memory_ISGG_no_relmem_v2.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
