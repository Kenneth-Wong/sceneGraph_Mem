...
from utils.snippets import compute_target, compute_hole_region, sub_hole_region
from model.config import cfg
...
class DualGraph_isc_pl_union_sub_isc(BaseMemory, Network):
    def __init__(self):
        self._n_iter = 2
        self._predictions = {}
    ...
    def _target_isc_layer(self, name):
        with tf.variable_scope(name):
            isc_rois, isc_batch_ids = tf.py_func(compute_target,
                                                 [self._feat_size, self._isc_rois,
                                                  self._feat_stride[0]],
                                                 [tf.float32, tf.int32],
                                                 name="target_isc_layer")
        ...
        return isc_rois, isc_batch_ids

    def _get_hole_region(self, name):
        with tf.variable_scope(name):
            hole_region = tf.py_func(compute_hole_region, [self._rel_rois, self._isc_rois, cfg.MEM.CROP_SIZE],
                                     tf.int32, name="hole_region")
            hole_region.set_shape([None, 5])
            self._targets['hole_region'] = hole_region
            self._score_summaries[0].append(hole_region)
        return hole_region

    def _union_sub_isc(self, rel_union_pool5, hole_region, name):
        with tf.variable_scope(name):
            rel_union_sub_isc = tf.py_func(sub_hole_region, [rel_union_pool5, hole_region], tf.float32, name="sub_hole_region")
            rel_union_sub_isc.set_shape([None, cfg.MEM.CROP_SIZE, cfg.MEM.CROP_SIZE, cfg.MEM.C])
            self._targets['rel_union_sub_isc'] = rel_union_sub_isc
            self._score_summaries[0].append(rel_union_sub_isc)
        return rel_union_sub_isc
    ...
    def _build_conv(self, is_training):
        ...
        with tf.variable_scope(self._scope, self._scope):
            ...
            isc_rois, isc_batch_ids = self._target_isc_layer("isc_target")
            hole_region = self._get_hole_region("hole_region")
            ...
            rel_union_pool5 = self._crop_rois(net_conv, rel_rois, rel_batch_ids, "rel_union_pool5")

            rel_union_sub_isc_pool5 = self._union_sub_isc(rel_union_pool5, hole_region, "rel_union_sub_isc_pool5")

            isc_pool5 = self._crop_rois(net_conv, isc_rois, isc_batch_ids, "isc_pool5")

            all_rel_pool5 = tf.concat([rel_union_sub_isc_pool5, isc_pool5], axis=3)
...
class vgg16(DualGraph_isc_pl_union_sub_isc, vgg16):
    def __init__(self):
        DualGraph_isc_pl_union_sub_isc.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(DualGraph_isc_pl_union_sub_isc, resnetv1):
...
class mobilenetv1(DualGraph_isc_pl_union_sub_isc, mobilenetv1):
    def __init__(self):
        DualGraph_isc_pl_union_sub_isc.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
