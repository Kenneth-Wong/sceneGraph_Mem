...
class DualGraph_relmix(BaseMemory, Network):
    def __init__(self):
        self._n_iter = 2
        self._predictions = {}
    ...
    def _build_conv(self, is_training):
        ...
        with tf.variable_scope(self._scope, self._scope):
            ...
            isc_pool5_nb = tf.stop_gradient(rel_pool5, name="isc_pool5_nb")
    ...
    def _parse_test_dict(self, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                     self._rois: blobs['rois'],
                     self._isc_rois: blobs['rel_rois'], # the trained model is like this
...
class vgg16(DualGraph_relmix, vgg16):
    def __init__(self):
        DualGraph_relmix.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
        self._scope = 'vgg_16'


class resnetv1(DualGraph_relmix, resnetv1):
...
class mobilenetv1(DualGraph_relmix, mobilenetv1):
    def __init__(self):
        DualGraph_relmix.__init__(self)
        self._feat_stride = [16, ]
        self._feat_compress = [1. / float(self._feat_stride[0]), ]
