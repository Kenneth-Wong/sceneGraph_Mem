...
class SolverWrapper(object):
    ...
    def initialize_from_npz(self, sess):
        # Initial file lists are empty
        np_paths = []
        ss_paths = []
        # Fresh train directly from ImageNet weights
        ...
        # Get the variables to restore, ignoring the variables to fix
        self.net.restore_variables_from_npz(sess, self.pretrained_model)
        print('Loaded.')
        # Need to fix the variables before loading, so that the RGB weights are changed to BGR
        # For VGG16 it also changes the convolutional weights fc6 and fc7 to
        # fully connected weights
    ...
    def initialize_nofix(self, sess):
        # Initial file lists are empty
        np_paths = []
        ss_paths = []
        # Fresh train directly from ImageNet weights
        ...
        var_keep_dic = self.get_variables_in_checkpoint_file(self.pretrained_model)
        # Get the variables to restore, ignoring the variables to fix
        variables_to_restore = self.net.get_variables_to_restore_nofix(variables, var_keep_dic)
    ...
    def train_model(self, sess, max_iters):
        ...
        self.data_layer = RoIDataLayer(self.imdb, self.roidb, self.bbox_means, self.bbox_stds, is_rel=False)
        self.data_layer_val = RoIDataLayer(self.imdb, self.roidb, self.bbox_means, self.bbox_stds, is_rel=False, random=True)
        ...
        while iter < max_iters + 1:
            ...
            if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
                ...
                out = self.net.train_step_with_summary(sess, blobs, train_op)
                summary = out['summary']
...
def train_net(network, imdb, roidb, valimdb, valroidb, output_dir, tb_dir,
    ...
    with tf.Session(config=tfconfig) as sess:
        sw = SolverWrapper(sess, network, imdb, roidb, valimdb, valroidb, output_dir, tb_dir,
